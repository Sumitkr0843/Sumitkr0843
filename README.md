<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&height=220&text=Sumit%20Kumar&fontSize=48&fontAlignY=40&color=0:4F46E5,50:7C3AED,100:9333EA" width="100%" />

<img src="https://readme-typing-svg.demolab.com?font=Inter&weight=600&size=28&duration=3000&pause=1000&color=8B5CF6&center=true&vCenter=true&width=900&lines=Software+Engineer;Java+Full+Stack+Developer;Spring+Boot+%7C+React+%7C+Cloud+Enthusiast;Building+Scalable+Enterprise+Applications" alt="Typing SVG" />

<br/>

![](https://img.shields.io/badge/B.Tech-CSE-7C3AED?style=for-the-badge)
![](https://img.shields.io/badge/GGSIPU-New%20Delhi-6366F1?style=for-the-badge)

![](https://img.shields.io/badge/Location-New%20Delhi,%20India-4F46E5?style=for-the-badge)

<a href="mailto:sumitkr5169@gmail.com">
<img src="https://img.shields.io/badge/Email-Contact-9333EA?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

<a href="https://linkedin.com/in/sumit-kumar-4b0604292/">
<img src="https://img.shields.io/badge/LinkedIn-Connect-6366F1?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="https://github.com/Sumitkr0843">
<img src="https://img.shields.io/badge/GitHub-Profile-4F46E5?style=for-the-badge&logo=github&logoColor=white"/>
</a>

<br/>

![](https://komarev.com/ghpvc/?username=Sumitkr0843&style=for-the-badge&color=7C3AED)
![](https://img.shields.io/github/followers/Sumitkr0843?style=for-the-badge&color=6366F1)
![](https://img.shields.io/github/stars/Sumitkr0843?affiliations=OWNER&style=for-the-badge&color=8B5CF6)

</div>

---

# About Me

Final-year **B.Tech Computer Science & Engineering** student passionate about building scalable software systems, enterprise-grade backend services, intelligent AI solutions, and modern web applications.

I specialize in:

- Enterprise Java Development
- Full Stack Engineering
- AI & Deep Learning Solutions
- REST API Design & Integration
- Cloud-Native Development
- Software Architecture & Security

### Open To

- Software Engineer Roles
- Backend Engineer Roles
- Full Stack Developer Roles
- Java Developer Positions
- AI/ML Engineering Opportunities
- Research & Product Engineering

---

# Tech Stack

### Languages

<p align="center">
<img src="https://skillicons.dev/icons?i=java,js,python,typescript" />
</p>

### Frontend

<p align="center">
<img src="https://skillicons.dev/icons?i=react,html,css,bootstrap,tailwind" />
</p>

### Backend & Databases

<p align="center">
<img src="https://skillicons.dev/icons?i=spring,nodejs,express,postgres,mysql,mongodb,redis" />
</p>

### Cloud, DevOps & Tooling

<p align="center">
<img src="https://skillicons.dev/icons?i=docker,git,github,maven,postman,vscode,idea" />
</p>

---

# Featured Projects

<details>
<summary><b>🚀 Feature Flag Management Platform</b></summary>

### Enterprise Feature Rollout System

| Category | Details |
|----------|----------|
| Stack | Java, Spring Boot, React.js, PostgreSQL, Redis |
| Scale | Multi-environment deployments |
| Performance | Redis-powered low latency access |
| Security | JWT Authentication |
| Impact | Safe production feature releases |
| Repository | Add Repository Link |

Built a dynamic feature management platform enabling percentage-based rollouts, audience targeting, audit logging, and deployment-safe feature releases without requiring application redeployment.

</details>

<details>
<summary><b>📡 API Monitoring Platform</b></summary>

### Real-Time Monitoring & Incident Tracking

| Category | Details |
|----------|----------|
| Stack | Java, Spring Boot, React.js, PostgreSQL |
| Scale | Multi-service monitoring |
| Performance | Automated health checks |
| Security | Secure API Management |
| Impact | Improved uptime visibility |
| Repository | Add Repository Link |

Designed an API observability platform featuring uptime analytics, latency dashboards, incident tracking, scheduled monitoring jobs, and alerting workflows.

</details>

<details>
<summary><b>🌱 Plant Disease Detection Using Deep Learning</b></summary>

### Springer Nature Accepted Research Project

| Category | Details |
|----------|----------|
| Stack | Python, PyTorch, MobileViT-XS, Grad-CAM, Streamlit |
| Scale | 29 Disease Classes |
| Performance | 99.34% Accuracy |
| Security | Controlled Inference Pipeline |
| Impact | Real-Time Agricultural Assistance |
| Repository | Add Repository Link |

Developed a hybrid CNN-Transformer architecture leveraging MobileViT-XS and transfer learning. Integrated Grad-CAM explainability and deployed a multilingual real-time prediction platform.

</details>

---

# Experience

## Frontend Development Intern — Prodesk IT Solution

**Duration:** Internship

### Responsibilities

- Built responsive user interfaces
- Improved frontend usability
- Worked on reusable UI components
- Collaborated within development teams

**Skills:** HTML • CSS • JavaScript • React • Responsive Design

---

## Java Full Stack Intern — EduSkills

**Duration:** Internship

### Responsibilities

- Developed backend APIs
- Built full stack applications
- Worked with Spring Boot ecosystem
- Integrated databases and authentication systems

**Skills:** Java • Spring Boot • REST APIs • PostgreSQL • React

---

# Achievements

| Recognition | Details |
|------------|---------|
| Research Publication | Springer Nature Accepted Paper |
| AI Research | Plant Disease Detection using Deep Learning |
| Full Stack Development | Enterprise-grade Java & React Projects |
| Engineering Portfolio | Multiple Production-Oriented Systems |

---

# GitHub Analytics

<div align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=Sumitkr0843&show_icons=true&theme=tokyonight&hide_border=true"/>

<img height="180em" src="https://github-readme-streak-stats.herokuapp.com/?Sumitkr0843&theme=tokyonight&hide_border=true"/>

<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Sumitkr0843&layout=compact&theme=tokyonight&hide_border=true"/>

</div>

---

# GitHub Trophies

<div align="center">

<img src="https://github-profile-trophy.vercel.app/?username=Sumitkr0843&theme=tokyonight&no-frame=true&row=1&column=7" />

</div>

---

# Contribution Activity

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=Sumitkr0843&theme=tokyo-night&hide_border=true" width="100%" />

</div>

---

# Contribution Snake

<div align="center">

<img src="https://raw.githubusercontent.com/Platane/snk/output/github-contribution-grid-snake-dark.svg" />

</div>

---

# Current Focus

```yaml
Learning:
  - Advanced System Design
  - Cloud Native Development
  - Distributed Systems

Building:
  - Enterprise SaaS Applications
  - AI Powered Solutions
  - Scalable Backend Services

Exploring:
  - Spring AI
  - Microservices Architecture
  - DevOps Automation

Open_To:
  - Software Engineering Roles
  - Backend Development
  - Full Stack Opportunities
```

---

# Connect

<div align="center">

<a href="mailto:sumitkr5169@gmail.com">
<img src="https://img.shields.io/badge/Gmail-Contact-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

<a href="https://linkedin.com/in/sumit-kumar-4b0604292/">
<img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="https://github.com/Sumitkr0843">
<img src="https://img.shields.io/badge/GitHub-Profile-181717?style=for-the-badge&logo=github&logoColor=white"/>
</a>


</div>

---

<div align="center">

### "Engineering reliable software today while building intelligent systems for tomorrow."

<img src="https://capsule-render.vercel.app/api?type=waving&section=footer&height=140&color=0:4F46E5,50:7C3AED,100:9333EA"/>

</div>
